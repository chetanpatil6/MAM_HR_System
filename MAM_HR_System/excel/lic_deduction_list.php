...
// Add some data
 ...
 $month_no = $_POST['cmb_lic_deduction_list_month'];
 $employee_no = array();
...
$objPHPExcel->setActiveSheetIndex(0)
			 ...
			 ->setCellValue("B2", "LIC Account No.")
...
while($r_payroll_details = mysql_fetch_assoc($sq_payroll_details))
{
 ...
 while($r_employment_dtls = mysql_fetch_assoc($sq_employment_dtls))
 {
    ...
    $objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A'.$i, $j)
				->setCellValue('B'.$i, $r_employment_dtls['lic_acc_no_1'])
				->setCellValue('C'.$i, $emp_name)
				->setCellValue('D'.$i, $r_employment_dtls['lic_amt_1']);
				
	$total_amt = $total_amt + $r_employment_dtls['lic_amt_1'];
	}
    ...
    $objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A'.$i, $j)
				->setCellValue('B'.$i, $r_employment_dtls['lic_acc_no_2'])
				->setCellValue('C'.$i, $emp_name)
				->setCellValue('D'.$i, $r_employment_dtls['lic_amt_2']);
				
	$total_amt = $total_amt + $r_employment_dtls['lic_amt_2'];
	}
...
$objPHPExcel->getActiveSheet()->setTitle('L.I.C. Deduction List');
...
header('Content-Disposition: attachment; filename="lic_deduction_list.xlsx"');
//read from server and write to buffer
readfile("lic_deduction_list.xlsx");
// Echo memory peak usage
