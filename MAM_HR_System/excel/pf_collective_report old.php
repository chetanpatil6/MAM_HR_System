...
$month_no1= date('F Y', strtotime($month_no));
...
//echo $month_no;
//echo $company_name;
if($company_name == 'maharashtra_arogya_mandal')
{
  $code = 'G2';
}
else
...
mysql_select_db("$company_name");

//$i++;

$sq_company_name = mysql_query("select * from company_info");
while($row_company_info = mysql_fetch_assoc($sq_company_name))
{
$row_company_info['name'] = str_replace('12345', ' ', $row_company_info['name']);
$company_name = $row_company_info['name'];
$company_address_address = $row_company_info['address1']." ".$row_company_info['address2']." ".$row_company_info['address3']." ".$row_company_info['address4'].", ".$row_company_info['city']." ".$row_company_info['pin_code'];
}
...
$objPHPExcel->setActiveSheetIndex(0)	
            ->setCellValue("C2",$company_address_address)
			->setCellValue("G2",$month_no1);			
...
$objPHPExcel->getActiveSheet()->getStyle('G2')->getFont()->setBold(true);
	...
	$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue("B3", "Account No.")
			->setCellValue("C3", "Name of Employee")
			->setCellValue("D3", "Basic")
			->setCellValue("E3", "DA")
			->setCellValue("F3", "Total")
			->setCellValue("G3", "HRA+Other")
			->setCellValue("H3", "Grand Total")
			->setCellValue("I3", "Employee Single Share")
			->setCellValue("J3", "15.67% A")
			->setCellValue("K3", "8.33% B")
			->setCellValue("L3", "24% (A+B)");
...
//$objPHPExcel->getActiveSheet()->setBreak( 'A3' , PHPExcel_Worksheet::BREAK_ROW );
$i = 4;	 
...
$sq_payroll_dtls = mysql_query("select distinct payroll_details.employee_no from payroll_details,comp_details where payroll_details.month_no='$month_no' and comp_details.type_of_sep='' and comp_details.pf_number!='' order by comp_details.designation");
...
$objPHPExcel->setActiveSheetIndex(0)	
            ->setCellValue("C".$i,$company_address_address)
			->setCellValue("G".$i,$month_no1);			
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue("B".$i, "Account No.")
			->setCellValue("C".$i, "Name of Employee")
			->setCellValue("D".$i, "Basic")
			->setCellValue("E".$i, "DA")
			->setCellValue("F".$i, "Total")
			->setCellValue("G".$i, "HRA+Other")
			->setCellValue("H".$i, "Grand Total")
			->setCellValue("I".$i, "Employee Single Share")
			->setCellValue("J".$i, "15.67% A")
			->setCellValue("K".$i, "8.33% B")
			->setCellValue("L".$i, "24% (A+B)");
