...
$unit_name = $row_company_info['name'];
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue("B4", "Account No.")
			->setCellValue("C4", "Name of Employee")
			->setCellValue("D4", "Basic")
			->setCellValue("E4", "DA")
			->setCellValue("F4", "Total")
			->setCellValue("G4", "HRA+Other")
			->setCellValue("H4", "Grand Total")
			->setCellValue("I4", "Employee Single Share")
			->setCellValue("J4", "15.67% A")
			->setCellValue("K4", "8.33% B")
			->setCellValue("L4", "24% (A+B)");
...
while($r_payroll_details = mysql_fetch_assoc($sq_payroll_dtls))
{
 ...
 // $grand_total = $total_basic_da + $total_other; ///////////////////////////////////////////////////////// Grand Total
  ...
  $c = $a + $b; ////////////////////////////////////////////////////////////////////////////////// 24%
    ...
    $emp_share_double = $pf * 2;
	if($emp_share_double != $c)
	{
		$c = $emp_share_double;
		$a = $c - $b;
	}
...
for($k=0;$k<sizeof($sr_no_array);$k++)
{
	...
	/*$emp_share_double = $pf_array[$k] * 2;
	if($emp_share_double != $c_array[$k])
	{
		$c_array[$k] = $emp_share_double;
		$a_array[$k] = $c_array[$k] - $b_array[$k];
	}
	*/
...
$sq_pf_collective = mysql_query("select * from pf_collective_report_sorted order by account_no");
$temp = 5;
$temp1 = 1;
...
for($arr=0;$arr<sizeof($arr_company_name);$arr++)
{
  ...
  $objPHPExcel->setActiveSheetIndex(0)
			...
			mysql_query("insert into pf_collective_report_summary values('$arr_company_name[$arr]','$arr_a1[$arr]','$arr_a2[$arr]','$arr_a3[$arr]','$arr_a4[$arr]','$arr_a5[$arr]','$arr_total_basic_da[$arr]')");
