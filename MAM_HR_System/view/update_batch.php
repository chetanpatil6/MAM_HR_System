...
<table width="900"><tr><td><font face="Courier New, Courier, monospace" size="+1" color="#FF6600"><center>Update Batch</center></font></td></tr></table>

<center><table class="listing" border="1" width="800">
  <?php
$sql = mysql_query("select * from batch_hdr where batch_no='$_POST[cmb_batch_no]'");
while($row = mysql_fetch_assoc($sql))
{
?>		
<tr><td><b>Batch No:</b><font color="#FF0000">*</font></td><td colspan="3"><input type="text" name="txt_batch_no" id="txt_batch_no" value="<?php echo $row['batch_no']; ?>" disabled="disabled" /></td>
</tr>
<tr>
<td><b>Month No:</b><font color="#FF0000">*</font></td><td colspan="3"><input type="text" name="txt_month_no" id="txt_month_no" value="<?php echo $row['month_no']; ?>"  disabled="disabled" /></td>
</tr>
<tr><td><b>Remark:</b></td><td colspan="3"><textarea name="txt_remark" id="txt_remark" cols="50" rows="3"><?php echo $row['remark'] ?></textarea></td>
</tr>
<?php
}
?>
<tr><td width="215"><b><center>Employee No.<font color="#FF0000">*</font></center></b></td><td width="210"><b><center>Compensation Code<font color="#FF0000">*</font></center></b></td><td width="145"><b><center>Adjustment Amount<font color="#FF0000">*</font></center></b></td><td><b><center>Remark</center></b></td></tr>
</table>


<table class="listing" border="1" width="800" id="tblbatchdtls" >
<?php
$sql6 = mysql_query("select * from batch_dtls where batch_no='$_POST[cmb_batch_no]'");
while($row1 = mysql_fetch_assoc($sql6))
{

?>
<tr>
<td><input type="text" name="txt_employee_no" id="txt_employee_no" value="<?php echo $row1['employee_no']; ?>" style="display:none"  /><?php 
$sql3 = mysql_query("select * from empl_master where employee_no='$row1[employee_no]'");
while($row2 = mysql_fetch_assoc($sql3))
{
?>
<input type="text" name="txt_employee_name" id="txt_employee_name" title="<?php echo $row2['first_name']." ".$row2['middle_name']." ".$row2['last_name']; ?>" value="<?php echo $row2['first_name']." ".$row2['middle_name']." ".$row2['last_name']; ?>" disabled="disabled" size="25" />
<?php
}	
?></td>
<td><select name="txt_comps_code" id="txt_comps_code" style="width:200px">
<?php
$sql5 = mysql_query("select * from comp_head where comp_code='$row1[comp_code]'");
while($row4 = mysql_fetch_assoc($sql5))
{
   echo "<option value='$row1[comp_code]' selected='selected'>$row1[comp_code] - $row4[comp_name]</option>";
}
$sql2 = mysql_query("select * from comp_head");
while($row3 = mysql_fetch_assoc($sql2))
{
  if($row1['comp_code'] != $row3['comp_code'])
  {
  	echo "<option value='$row3[comp_code]'>$row3[comp_code] - $row3[comp_name]</option>";
  }
}
?>
</select> <!--<input type="text" name="txt_comps_code" id="txt_comps_code" /> --></td>
<td><input type="text" name="txt_adjustment_amount" id="txt_adjustment_amount" size="15" value="<?php echo $row1['adj_amount']; ?>" /></td>
<td><input type="text" name="txt_remark_any" id="txt_remark_any" size="25" value="<?php echo $row1['remark_for_adj']; ?>" /></td>
</tr>
<?php
}
?>
...
<tr><td colspan="8"><center><input type="button" name="btn_update_batch" id="btn_update_batch" value="Update" onclick="update_batch();" /><input type="reset" name="btn_reset" value="Cancel" /></center></td></tr>
