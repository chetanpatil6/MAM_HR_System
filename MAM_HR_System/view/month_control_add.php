...
function validation_month(obj){
    
	var date = new Date();
	var selected_date = document.getElementById('txt_month_no1').value;
	
	
	var dd = date.getDate()+1;
	var mm = date.getMonth()+1;
	var yyyy = date.getFullYear();
	var today_date = yyyy+"-"+mm;
	
	if(new Date(selected_date) < new Date(today_date))
	{
		alert("Can not select Previous Months");
		//alert(selected_date+today_date);
		document.getElementById('txt_month_no1').value = "";
		return false;
	}
	else
	{
...
$sq_month = mysql_query("select month_no from month_control where month_no='$current_month'");
if(mysql_num_rows($sq_month) > 0)
{
  $month = $prev_month = date('Y-m', strtotime($current_month." +1 month"));
}
else
{
   $month = $current_month;
...
$sq_month = mysql_query("select month_no from month_control order by month_no desc");
if($r_month = mysql_fetch_assoc($sq_month)){
    $month = date('Y-m', strtotime($r_month['month_no']." +1 month"));
}else{
    $month = date('Y-m');
}
...
<tr><td><b>Month No:</b><font color="#FF5F55">*</font></td><td colspan="4"><!--<input type="text" name="txt_month_no1_display" id="txt_month_no1_display" value="<?php echo $month; ?>" disabled="disabled" /> -->
<input type="text" name="txt_month_no1" id="txt_month_no1" onblur="validation_month(this.value)" value="<?php echo $month; ?>" />

<button id="f_btn1">...</button></td>
</tr>
<tr><td><b>Month Active:</b><font color="#FF5F55">*</font></td><td><b>Active</b><input type="radio" name="rdb_month_avtive" id="rdb_month_avtive" value="Y" /></td><td><b>Closed</b><input type="radio" name="rdb_month_avtive" id="rdb_month_avtive" value="N"  checked="checked" /></td>
</tr>
<?php
$sq_index = mysql_fetch_assoc(mysql_query("SELECT * FROM month_control ORDER BY month_no DESC LIMIT 1"));
?>
...
<tr><td><b>Dearness Allowance Index</b></td><td colspan="3"><input type="text" name="txt_da_index" id="txt_da_index" value="<?php echo $sq_index['da_index']; ?>" /><b>%&nbsp;&nbsp;</b><label><font size="-3" color="#990000">(Default Values are from Previous Month.)</font></label></td></tr>


<tr><td><b>H.R.A. Index</b></td><td colspan="3"><input type="text" name="txt_hra_index" id="txt_hra_index" value="<?php echo $sq_index['hra_index']; ?>" /><b>%&nbsp;&nbsp;</b><label><font size="-3" color="#990000">(Default Values are from Previous Month.)</font></label></td></tr>

<tr><td><b>Bonus Index</b></td><td colspan="3"><input type="text" name="txt_bonus_index" id="txt_bonus_index" value="0" /><b>%</b></td></tr>
	 
<tr><td colspan="5"><center><input type="button" name="btn_add_month_control" id="btn_add_month_control" value="Save" onclick="numberOfDays();add_month_control();" /><input type="reset" name="btn_reset" value="Cancel" /></center></td></tr>
