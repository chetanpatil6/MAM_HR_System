...
function leave_dashboard_info()
{
	...
	xmlhttp.open("GET","leaves_dashboard_filter.php?project_id="+emp_no+"&leave_type="+leave_type+"&from_date="+from_date+"&to_date="+to_date+"&status="+status,true); 
	xmlhttp.send();
}
...
<table width="900"><tr><td><font face="Courier New, Courier, monospace" size="+1" color="#FF6600"><center>Leave Dashboard</center></font></td></tr></table>
...
<td>
<b><font color="#000000">From Date: </font> </b></td><td>
<select style="width:120px" name="txt_from_date" id="txt_from_date" onchange="leave_dashboard_info()">
<option value="select">Select</option>
<?php
$sq_trans_date = mysql_query("select distinct from_date from leave_management");
$count_leaves_type = mysql_num_rows($sq_trans_date);
while($res_leaves_type = mysql_fetch_assoc($sq_trans_date))
{
 echo '<option value="'.$res_leaves_type['from_date'].'">'.strtoupper($res_leaves_type['from_date']).'</option>';
}
?>
...
<td><b><font color="#000000">To Date: </font></b></td>
<td><select style="width:120px" name="txt_to_date" id="txt_to_date" onchange="leave_dashboard_info()">
<option value="select">Select</option>
<?php
$sq_trans_date = mysql_query("select distinct to_date from leave_management");
$count_leaves_type = mysql_num_rows($sq_trans_date);
while($res_leaves_type = mysql_fetch_assoc($sq_trans_date))
{
 echo '<option value="'.$res_leaves_type['to_date'].'">'.strtoupper($res_leaves_type['to_date']).'</option>';
}
?>
...
<td><b>Status &nbsp;<font color="#FF0000">*</font></b></td><td><select name="txt_status" id="txt_status" style="width:200px" onchange="leave_dashboard_info();">
<option value="select">Select</option>
<?php
	$status = mysql_query("select distinct leave_status from leave_management");
	while($row=mysql_fetch_assoc($status))
	{
	 echo"<option value='$row[leave_status]'>$row[leave_status]</option>";
	}
	?>
</select>
</td>
...
<td><center><font color="#CC0000">App Number</font></center></td>
<td><center><font color="#CC0000">App Date</font></center></td>
<td><center><font color="#CC0000">Designation</font></center></td>
<td><center><font color="#CC0000">Form Date</font></center></td>
<td><center><font color="#CC0000">To Date</font></center></td>
<td><center><font color="#CC0000">Total Days</font></center></td>
<td><center><font color="#CC0000">Leave Status</font></center></td>
<td><center><font color="#CC0000">Type</font></center></td>
</tr>
</thead>
<tbody>
...
$sq1="select * from leave_management ";
