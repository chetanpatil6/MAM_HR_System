 ...
 for (var i=0; i < document.company_info.chk_pf_applicable.length; i++)
   ...
   if (document.company_info.chk_pension_fund_applicable[i].checked)
      ...
      var pension_fund_applicable = document.company_info.chk_pension_fund_applicable[i].value;
	   ...
	   var pf_uan_no = $("#txt_pf_uan_no").val();
	   var pf_uan_date = $("#txt_pf_uan_date").val();
...
$.post('../controller/add_company_info.php', {emp_no:emp_no,date_of_joining:date_of_joining, comp_change_eff_date:comp_change_eff_date, designation:designation,grade:grade, basic:basic, hra:hra, special:special, conveyance:conveyance,lta:lta, pf_applicable:pf_applicable, prof_tax_applicable:prof_tax_applicable, esi_applicable:esi_applicable,bank_branch_name:bank_branch_name,account_no:account_no,pf_no:pf_no,pf_date:pf_date,pf_uan_no:pf_uan_no,pf_uan_date:pf_uan_date,dearness_pay:dearness_pay,da:da,gross_salary:gross_salary,ctc:ctc,emp_type:emp_type,emp_cat:emp_cat,date_confirm:date_confirm,date_sep:date_sep,type_sep:type_sep,remark_sep:remark_sep,salary_on_joining:salary_on_joining,mode_of_payment:mode_of_payment,gratuity_applicable:gratuity_applicable,tds_applicable:tds_applicable,pension_fund_applicable:pension_fund_applicable, lic_acc_1:lic_acc_1, lic_amt_1:lic_amt_1, lic_acc_2:lic_acc_2, lic_amt_2:lic_amt_2, loan_acc_1:loan_acc_1, loan_amt_1:loan_amt_1, loan_acc_2:loan_acc_2, loan_amt_2:loan_amt_2, lic_id_1:lic_id_1, lic_id_2:lic_id_2, lic_gratuity_1:lic_gratuity_1, lic_gratuity_2:lic_gratuity_2, loan_id_1:loan_id_1, loan_gratuity_1:loan_gratuity_1, loan_id_2:loan_id_2, loan_gratuity_2:loan_gratuity_2, grant_type:grant_type, department:department}, function(data){
				$('#error2').text(data);
});
}
...
function add_attendance(){
	...
	validateattendance();
	 ...
	 var income_tax = new Array();
		...
		var txtBox11 = row.cells[11].childNodes[0].value;
		...
		var total = parseFloat(txtBox3) + parseFloat(txtBox7) + parseFloat(txtBox8) + parseFloat(txtBox9) + parseFloat(txtBox10);
		if(total != 0)
		{
...
$.post('../controller/add_attendance.php',{'empl_no[]':empl_no, 'month_nob[]':month_nob, 'per_day_for_month[]':per_day_for_month,'holidays[]':holidays, 'paid_leaves[]':paid_leaves, 'unpaid_leaves[]':unpaid_leaves, 'adj_days[]':adj_days, 'other_Hrs[]':other_Hrs, 'leave_encash_days[]':leave_encash_days, 'suspention_days[]':suspention_days, 'income_tax[]':income_tax}, function(data){
				$('#error5').text(data);
});
}
...
function update_attendance(){
 ...
 counter = 0;			
		for(var i=0; i<rowCount; i++) {
			 ...
			 if(parseFloat(total) > 0 && parseFloat(total) != parseFloat(no_of_days))
			 {
...
$.post('../controller/update_attendance.php',{'empl_no[]':empl_no, 'month_nob[]':month_nob, 'per_day_for_month[]':per_day_for_month, 'holidays[]':holidays, 'paid_leaves[]':paid_leaves, 'unpaid_leaves[]':unpaid_leaves, 'adj_days[]':adj_days,'other_Hrs[]':other_Hrs,'leave_encash_days[]':leave_encash_days,'suspention_days[]':suspention_days,'income_tax[]':income_tax}, function(data){
				$('#error5').text(data);
});
}
...
function add_leave(){
       var doc_no = $("#txt_doc_no").val();
	   var doc_date = $("#txt_doc_date").val();
	   var attendance_no = $("#txt_attendance_no").val();
	   var leave_application_number = $("#txt_leave_application_number").val();
	   var registration_no = $("#txt_registration_no").val();
	   var emp_no= $("#cmb_to_employee_name").val();
	   //alert("test");
	   if(emp_no == 'select')
	   {
	   ...
	   {
			alert("Registration Number will be numbers");   
			return false;
	   }
	...
	if(application_date=="" || application_date==null){return false;}								
	
	// m[1] is year 'YYYY' * m[2] is month 'MM' * m[3] is day 'DD'					
	var m = application_date.match(/(\d{4})-(\d{2})-(\d{2})/);
	
	// STR IS NOT FIT m IS NOT OBJECT
	if( m === null || typeof m !== 'object'){
		 alert("Please Enter Proper Format For Application Date");
		return false;
		}				
	...
	if( (m[1].length < 2) || m[3] < 1 || m[3] > 31){
	   ...
	   var received_date = $("#txt_received_date").val();
	   
	   if(received_date!=''){
	   
	    // STRING FORMAT yyyy-mm-dd
	if(received_date=="" || received_date==null){return false;}								
	
	// m[1] is year 'YYYY' * m[2] is month 'MM' * m[3] is day 'DD'					
	var m = received_date.match(/(\d{4})-(\d{2})-(\d{2})/);
	
	// STR IS NOT FIT m IS NOT OBJECT
	if( m === null || typeof m !== 'object'){
		 alert("Please Enter Proper Format For Application Received Date");
		return false;
		}				
	...
	if( (m[1].length < 2) || m[3] < 1 || m[3] > 31){
	   ...
	   // STRING FORMAT yyyy-mm-dd
	if(from_date=="" || from_date==null){return false;}								
	
	// m[1] is year 'YYYY' * m[2] is month 'MM' * m[3] is day 'DD'					
	var m = from_date.match(/(\d{4})-(\d{2})-(\d{2})/);
	
	// STR IS NOT FIT m IS NOT OBJECT
	if( m === null || typeof m !== 'object'){
		 alert("Please Enter Proper Format For From Date");
		return false;
		}				
	...
	if( (m[1].length < 2) || m[3] < 1 || m[3] > 31){
	   ...
	   var to_date = $("#txt_to_date").val();
	    ...
	    // STRING FORMAT yyyy-mm-dd
	if(to_date=="" || to_date==null){return false;}								
	
	// m[1] is year 'YYYY' * m[2] is month 'MM' * m[3] is day 'DD'					
	var m = to_date.match(/(\d{4})-(\d{2})-(\d{2})/);
	
	// STR IS NOT FIT m IS NOT OBJECT
	if( m === null || typeof m !== 'object'){
		 alert("Please Enter Proper Format For To Date");
		return false;
		}				
	...
	if( (m[1].length < 2) || m[3] < 1 || m[3] > 31){
	   ...
	   var from_day = '';
       			 ...
       			 from_day = document.frm_leave_management.rdb_from_day.value;
			
		 
		 var to_day = '';
		
   			
       			 to_day = document.frm_leave_management.rdb_to_day.value;
				
     // alert("test");
  		// if(to_day == '' || from_day == '')
  		 {
    		// alert("Select Half day or Full Day");
			 //return false;
   		 }
	   
	
  
	   var leave_type = $("#cmb_leave_type").val();
	   ...
	   var total_leave = $("#txt_total_leaves").val();
	   var stock_type = $("#txt_stock_type").val();
	   var updated_by = $("#txt_updated_by").val();
	   var leave_status = $("#txt_leave_status").val();
	   
	   var remark = $("#txt_remark").val();
	   var count_leave_type = $("#txt_count_leave_type").val();
	   
	    var temp_count = 1;		
		for(var i=0; i<count_leave_type; i++)
		{
			
			var current_leaves = $("#txt_current_leaves"+temp_count).val();			
			var current_leaves_code = $("#txt_current_leaves_code"+temp_count).val();		
			
			if((leave_type==current_leaves_code) && (parseFloat(total_leave) > parseFloat(current_leaves)))
		    {
				alert("Leaves can not be greater than balace leaves");
				return false;
			}	
	  ...
	  //return false;
  //alert("test"); 
$.post('../controller/add_leave.php', {doc_no:doc_no,doc_date:doc_date,emp_no:emp_no,leave_type:leave_type,attendance_no:attendance_no,leave_application_number:leave_application_number,registration_no:registration_no,application_date:application_date,designation:designation,received_date:received_date,from_date:from_date,from_day:from_day,to_date:to_date,to_day:to_day,total_leave:total_leave,stock_type:stock_type,updated_by:updated_by,leave_status:leave_status,remark:remark}, function(data){
				$('#error').text(data);
});
}
...
function update_leave(){
	   ...
	   var total_leave = $("#txt_total_leave").val();
	   var cl = $("#txt_cl").val();
	   var fl = $("#txt_fl").val();
	   var el = $("#txt_el").val();
	   var ml = $("#txt_ml").val();
	   var sp = $("#txt_sp").val();
	   var unpaid_leaves = $("#txt_unpaid_leaves").val();
	   var absent_days = $("#txt_absent_days").val();
	   var remark = $("#txt_remark").val();
   
$.post('../controller/update_leave.php', {emp_no:emp_no,attendance_no:attendance_no, leave_application_number:leave_application_number, registration_no:registration_no, application_date:application_date, designation:designation, from_date:from_date, to_date:to_date,month:month, total_leave:total_leave, cl:cl, fl:fl,el:el, ml:ml, sp:sp, unpaid_leaves:unpaid_leaves, absent_days:absent_days,remark:remark}, function(data){
				$('#error').text(data);
});
}
...
function cancel_leave(){
	   ...
	   var from_date = $("#txt_from_date").val();
	   var to_date = $("#txt_to_date").val();
	   var leave_status = $("#txt_status").val();
	    var total_leav = $("#txt_total_leav").val();
...
$.post('../controller/cancel_leave.php', {attendance_no:attendance_no, leave_application_number:leave_application_number, registration_no:registration_no, emp_no:emp_no, application_date:application_date, designation:designation, from_date:from_date, to_date:to_date,leave_status:leave_status, total_leav:total_leav}, function(data){
				$('#error').text(data);
...
function add_leave_attendance()
{
	...
	//alert("test");
		...
		var emp_no = new Array();
		var month = new Array();
		var no_day_month = new Array();
		var week_off = new Array();
		var working_days = new Array();
		var present_days = new Array();
		var paid_days = new Array();
		var unpaid_days = new Array();
		var absent_days = new Array();
		var suspen_days = new Array();
		var encashment_days = new Array();
	    ...
	    for(var i=1; i<rowCount; i++) 
		{
                var row = table.rows[i];
	 			...
	 			var txtBox0 = row.cells[0].childNodes[0].value;
				...
				var txtBox3 = row.cells[4].childNodes[0].value;
				//alert("test");
				var txtBox4 = row.cells[5].childNodes[0].value;
				var txtBox5 = row.cells[6].childNodes[0].value;
				var txtBox6 = row.cells[7].childNodes[0].value;
				var txtBox7 = row.cells[8].childNodes[0].value;
				
				var txtBox8 = row.cells[9].childNodes[0].value;
				var txtBox9 = row.cells[10].childNodes[0].value;
				var txtBox10 = row.cells[11].childNodes[0].value;
				var txtBox11 = row.cells[12].childNodes[0].value;
				
				  emp_no.push(txtBox0);
				  month.push(txtBox1);
				  no_day_month.push(txtBox2);
				  week_off.push(txtBox3);
				  working_days.push(txtBox4);
				  present_days.push(txtBox5);
				  paid_days.push(txtBox6);
				  unpaid_days.push(txtBox7);
				  
				  absent_days.push(txtBox8);
				  suspen_days.push(txtBox9);
				  encashment_days.push(txtBox10);
				  remark.push(txtBox11);
	  ...
	  else
	  {
		...
		//alert(encashment_days);
		
$.post('../controller/add_leave_attendance.php', {'emp_no[]':emp_no, 'month[]':month, 'no_day_month[]':no_day_month,'week_off[]':week_off, 'working_days[]':working_days, 'present_days[]':present_days, 'paid_days[]':paid_days, 'unpaid_days[]':unpaid_days, 'absent_days[]':absent_days, 'suspen_days[]':suspen_days, 'encashment_days[]':encashment_days, 'remark[]':remark}, function(data){
																																																																																					  //alert("test"); 
				$('#error').text(data);
});
}

function update_leave_attendance()
{
...
$.post('../controller/update_leave_attendance.php', {'emp_no[]':emp_no, 'month[]':month, 'no_day_month[]':no_day_month,'week_off[]':week_off, 'working_days[]':working_days, 'present_days[]':present_days, 'paid_days[]':paid_days, 'unpaid_days[]':unpaid_days, 'absent_days[]':absent_days, 'suspen_days[]':suspen_days, 'encashment_days[]':encashment_days, 'remark[]':remark}, function(data){
																																																																																					  //alert("test"); 
				$('#error').text(data);
});
}
...
function add_leave_transfer_type()
{
       
	   var employee_name = $("#cmb_to_employee_name").val();
	   var transfer_from = $("#cmb_transfer_from").val();
	   var leaves_transfer = $("#txt_leaves_transfer").val();
	   var transfer_to= $("#cmb_transfer_to").val();
	   
	   var count_leave_type = $("#txt_count_leave_type").val();
	   var current_total_leaves = $("#txt_current_total_leave").val();
	   var created_date = $("#txt_date").val();
	   if(employee_name == 'select')
   {
   		alert("Please select employee");
		return false;
   }
	   if(transfer_from == 'select')
	   {
		   alert("Please select Transfer from");
		   return false;
	   }
	   
	   if(transfer_to == 'select')
	   {
	   ...
	   {
		   alert("This leave transfer not allowed");
		   return false;
	   }
	   
	   if(leaves_transfer == '')
	   {
		   alert("Please select Leaves To Transfer");
		   return false;
	   }

	   if(leaves_transfer <= 0)
	   {
		   alert("Please select Leaves To Transfer Greater Than Zero");
		   return false;
	   }
	   
	   if(isNaN(leaves_transfer))
	   {
		...
		var currentLeaveCodeArray = new Array();
		//alert(count_leave_type);
		for(var i=0; i<count_leave_type; i++)
		{
			
			var current_leaves = $("#txt_current_leaves"+temp_count).val();			
			var current_leaves_code = $("#txt_current_leaves_code"+temp_count).val();			
			leaveArray.push(current_leaves);
			currentLeaveCodeArray.push(current_leaves_code);
			temp_count++;
			
			if((transfer_from==current_leaves_code) && (parseFloat(leaves_transfer) > parseFloat(current_leaves)))
		    {
				alert("Leaves to transfer must not be greater than current leaves");
				return false;
			}			
	  ...
	  else
	  {
		...
		//alert(leaveArray + currentLeaveCodeArray);
		//return false;
	 /*  var current_cl = $("#txt_current_cl").val();
	   var current_fl = $("#txt_current_fl").val();
	   var current_el= $("#txt_current_el").val();
	   var current_ml= $("#txt_current_ml").val();
	   var current_sp = $("#txt_current_sp").val();*/
	   /*var current_total_leaves = $("#txt_current_total_leave").val();
	   var created_date = $("#txt_date").val();
	   if(transfer_from=='cl' && leaves_transfer > current_cl)
	   {
		alert("Leaves to transfer must not be greater than current leaves in cl");
		return false;
	    }
	   if(transfer_from=='fl' && parseFloat(leaves_transfer) > parseFloat(current_fl))
	   {
		alert("Leaves to transfer must not be greater than current leaves in fl");
		return false;
	    }
		 if(transfer_from=='ml' && parseFloat(leaves_transfer) > parseFloat(current_ml))
	   {
		alert("Leaves to transfer must not be greater than current leaves in ml");
		return false;
	    }
		 if(transfer_from=='sp' && parseFloat(leaves_transfer) > parseFloat(current_sp))
	   {
		alert("Leaves to transfer must not be greater than current leaves");
		return false;
	    }*/
  // return false;
$.post('../controller/add_leave_transfer_type.php', {employee_name:employee_name,transfer_from:transfer_from, leaves_transfer:leaves_transfer, transfer_to:transfer_to, 'leaveArray[]':leaveArray,'currentLeaveCodeArray[]':currentLeaveCodeArray,current_total_leaves:current_total_leaves,created_date:created_date}, function(data){
				$('#error').text(data);
});
}
...
function add_employee_leaves()
{
        var emp_no = new Array();		
		var leaveArray = new Array();
		
	   var leaveCodeArray = new Array();	 
	   var table = document.getElementById('tableDisplay');
       var rowCount = table.rows.length;		  
	   var count=1;	   
	   var temp_count = 1;
	   var count_type = document.getElementById('txt_count_leave_type').value;		   
	  //alert(count_type+"test"+rowCount);
       for(var i=1; i<rowCount; i++) 
	   {        	   
			//var row = table.rows[i];
			var working_day = 0;								  
			var txtBox0 = document.getElementById('txt_emp_no'+count).value;			
			var temp_count1 = 1;			
			for(var j=0; j<count_type; j++)
			{
				
				var onhand = document.getElementById('txt_emp_leave_type_onhand'+count+temp_count1).value;				
				var leave_type = document.getElementById('txt_emp_leave_type'+count+temp_count1).value;
				
				
				if(onhand =='')
				{
				alert("Please enter all leaves");
				return false;
				}
				leaveCodeArray.push(leave_type);				
				leaveArray.push(onhand);				
				//alert(leaveArray);
				temp_count1++;					
			}			
			emp_no.push(txtBox0);				
			count++;
			//alert(leaveCodeArray+leaveArray+emp_no);
	   }
	  ...
	  else
	  {
	   ...
	   //alert("test"+count_type);
				//return false;
//alert(emp_no);
//return false;
$.post('../controller/add_employee_leaves.php', {'emp_no[]':emp_no, 'leaveArray[]':leaveArray, 'leaveCodeArray[]':leaveCodeArray,count_type:count_type}, function(data){
																																			//alert("test");
				$('#error').text(data);
});
}
...
function remove_employee_leaves()
{
        var emp_no = new Array();		
		var leaveArray = new Array();
...
//alert(emp_no);
//return false;
$.post('../controller/remove_employee_leaves.php', {'emp_no[]':emp_no, 'leaveArray[]':leaveArray, 'leaveCodeArray[]':leaveCodeArray,count_type:count_type}, function(data){
																																			//alert("test");
				$('#error').text(data);
});
}
...
/////////////////////////////////////////////////////////////////////////////End leave Managment//////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////start leave encashment////////////////////////////////////////////////
function add_leave_encashment_form()
{    
        var table = document.getElementById('tableDisplay');
        var rowCount = table.rows.length;	
			 
		var emp_no = new Array();	
		var currentelArray = new Array();
		var balancedelArray = new Array();
		var totalelArray = new Array();
	    for(var i=1; i<rowCount; i++) 
		{
                var row = table.rows[i];	   
	 			var txtBox0 = row.cells[0].childNodes[0].value;				
				...
				var txtBox3 = row.cells[4].childNodes[0].value;
				  ...
				  emp_no.push(txtBox0);
				  currentelArray.push(txtBox1);
				  balancedelArray.push(txtBox2);
				  totalelArray.push(txtBox3);
				//  alert(txtBox1);
	 ...
	 // alert(totalelArray);
$.post('../controller/add_leave_encashment_form.php', {'emp_no[]':emp_no, 'currentelArray[]':currentelArray, 'balancedelArray[]':balancedelArray, 'totalelArray[]':totalelArray}, function(data){
				$('#error').text(data);
				
});
}
//////////////////////////////////////////////////////end leave encashment/////////////////////////////////////////////////////////////////////////////
