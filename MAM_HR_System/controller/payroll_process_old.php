...
/////////////  For deductions ////////////
	...
	if($num_r_sq_payroll_chk == 0)
	   ...
	   $paid_amt = $total_pay_days + $adj_amt;
...
//////////// For Batches ////////////
	...
	$sql_batch = mysql_query("select * from batch_hdr where month_no='$month_number'");
	while($rw_batch = mysql_fetch_assoc($sql_batch))
	{
	  $sql_batch_details = mysql_query("select * from batch_dtls where batch_no='$rw_batch[batch_no]' and employee_no='$r[employee_no]'");
	  while($rw_batch_details = mysql_fetch_assoc($sql_batch_details))
	  {
	     $sq_payroll = mysql_query("select * from payroll_details where employee_no='$r[employee_no]' and comp_code='$rw_batch_details[comp_code]' and month_no='$month_number'");
		$num_sq_payroll = mysql_num_rows($sq_payroll);
		if($num_sq_payroll == 0)
		{
		    $sq_insert_into_payroll = mysql_query("insert into payroll_details values('$r[employee_no]', '$month_number', '$rw_batch_details[comp_code]', '', '', '', '$rw_batch_details[adj_amount]', '$tdate', '$_SESSION[txt_login_name]', '', '')");
			
			$comp_code_type = substr($rw_batch_details['comp_code'], 0, 1);
			
			
			if($comp_code_type == 'E')
			{
			  $tot_earnings = $tot_earnings + $rw_batch_details['adj_amount'];
			}
			else if($comp_code_type == 'D')
			{
			  $tot_deductions = $tot_deductions + $rw_batch_details['adj_amount'];
			}
			else if($comp_code_type == 'F')
			{
			  if($rw_batch_details['adj_amount'] < 0)
			  {
			    	$tot_deductions = $tot_deductions + $rw_batch_details['adj_amount'];
			  }
			  else
...
////////// For Recovery ///////////
	...
	while($r_recov = mysql_fetch_assoc($sq_recov))
	{
	   ...
	   {
 			 $sq4 = mysql_query("insert into payroll_details values('$r_recov[employee_no]', '$month_number', '$r_recov[comp_code]', '', '', '', '-$amt', '$tdate', '$_SESSION[txt_login_name]', '', '')");
  	   }
...
/*	if($current_month == '02')
	...
	{*/
		if($pt_amt >= 4999 && $pt_amt <= 9999)
		{
	   		$tot_pt = 175;
		}
		else if($pt_amt >= 10000)
		{
	   		$tot_pt = 200;
		}
		else
...
/*if($status == 'complete')
{
$sq = mysql_query("select * from payroll_hdr where month_no='$month_number'");
while($r = mysql_fetch_assoc($sq))
{
  $sq1 = mysql_query("select * from recovery where employee_no='$r[employee_no]'");
  while($r1 = mysql_fetch_assoc($sq1))
  { 
    if($r1['recov_status'] == 'Open')
	{
     $net_pay = $r['net_salary'];
	 $balance_amount = $r1['balance_amount'] - $r1['inst_amount'];
	 
	// $count = $r1['count'] - 1;
	 $sq2 = mysql_query("update recovery set balance_amount='$balance_amount' where employee_no='$r1[employee_no]'"					);
	  
	 if($count == 0)
	 {
	   $sq3 = mysql_query("update recovery set recov_status='Close' where employee_no='$r1[employee_no]'");
	 }
	}
  }
}

//$q3 = mysql_query("update month_control set month_active='N' where month_no='$month_number'");
}*/
